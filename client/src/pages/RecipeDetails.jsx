...
const RecipeDetails = () => {
  ...
  const getBlogDetail = async () => {
    try {
      const { data } = await axios.get(`http://localhost:7800/api/recepieBlogs/get-recepie${id}`);
      if (data?.success) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.put(`http://localhost:7800/api/recepieBlogs/update-recepie/${id}`, {
        title: inputs.title,
